import { BreakoutSignal, Candle, ScalpDirection, TrendLine } from './types';
...
const DEFAULT_CONFIG: TrendBreakConfig = {
  max5mCandles: 120,
  max1mCandles: 180,
  pivotSpan: 2,
  minPivots: 3,
...
export class TrendBreakEngine {
  private readonly config: TrendBreakConfig;
  private readonly candles5m = new Map<string, Candle[]>();
  private readonly candles1m = new Map<string, Candle[]>();

  constructor(config: Partial<TrendBreakConfig> = {}) {
    this.config = { ...DEFAULT_CONFIG, ...config };
  }

  public ingest(candle: Candle): void {
  ...
  public detectBreakout(symbol: string, direction: ScalpDirection): BreakoutSignal | null {
    const five = this.candles5m.get(symbol) ?? [];
    const one = this.candles1m.get(symbol) ?? [];
    if (five.length < 10 || one.length < this.config.volumeLookback + 1) return null;

    const line = direction === 'LONG'
      ? this.fitTrendLine(five, 'DESCENDING_RESISTANCE')
      : this.fitTrendLine(five, 'ASCENDING_SUPPORT');
    if (!line || line.r2 < this.config.minTrendR2) return null;
  ...
  public getTrendLine(symbol: string, direction: ScalpDirection): TrendLine | null {
    const five = this.candles5m.get(symbol) ?? [];
  ...
  private fitTrendLine(candles: Candle[], kind: TrendLine['kind']): TrendLine | null {
    const pivots = kind === 'DESCENDING_RESISTANCE'
      ? this.pivotHighs(candles)
      : this.pivotLows(candles);
    if (pivots.length < this.config.minPivots) return null;
    ...
    if (kind === 'DESCENDING_RESISTANCE' && !descending) return null;
    if (kind === 'ASCENDING_SUPPORT' && !ascending) return null;
    ...
    if (kind === 'DESCENDING_RESISTANCE' && fit.slope >= 0) return null;
    if (kind === 'ASCENDING_SUPPORT' && fit.slope <= 0) return null;
  ...
  private pivotHighs(candles: Candle[]): Array<{ timestamp: number; price: number }> {
    const out: Array<{ timestamp: number; price: number }> = [];
    const s = this.config.pivotSpan;
    for (let i = s; i < candles.length - s; i++) {
      const v = candles[i].high;
      let ok = true;
      for (let j = i - s; j <= i + s; j++) if (j !== i && candles[j].high >= v) ok = false;
      if (ok) out.push({ timestamp: candles[i].timestamp, price: v });
    }
    return out;
  }

  private pivotLows(candles: Candle[]): Array<{ timestamp: number; price: number }> {
    const out: Array<{ timestamp: number; price: number }> = [];
    const s = this.config.pivotSpan;
    for (let i = s; i < candles.length - s; i++) {
      const v = candles[i].low;
      let ok = true;
      for (let j = i - s; j <= i + s; j++) if (j !== i && candles[j].low <= v) ok = false;
      if (ok) out.push({ timestamp: candles[i].timestamp, price: v });
    }
    return out;
  }

  private project(line: TrendLine, timestamp: number): number {
