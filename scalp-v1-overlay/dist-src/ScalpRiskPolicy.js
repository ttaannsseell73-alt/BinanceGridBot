...
exports.ScalpRiskPolicy = void 0;
...
class ScalpRiskPolicy {
    ...
    size(req) {
        ...
        if (req.realizedPnlTodayUsd <= -this.config.dailyMaxLossUsd)
            return fail('DAILY_LOSS_LIMIT');
        if (req.currentOpenRiskUsd + this.config.riskPerTradeUsd > this.config.maxOpenRiskUsd)
            return fail('MAX_OPEN_RISK');
        if (!(req.entryPrice > 0) || !(req.stopPrice > 0) || req.entryPrice === req.stopPrice)
            return fail('INVALID_ENTRY_STOP');
        const stopDistancePct = Math.abs(req.entryPrice - req.stopPrice) / req.entryPrice * 100;
        if (stopDistancePct < this.config.minStopDistancePct)
...
exports.ScalpRiskPolicy = ScalpRiskPolicy;
