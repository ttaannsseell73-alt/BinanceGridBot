import { RadarCandidate, ScalpDirection, TradeTick } from './types';

export interface FuturesRadarConfig {
  windowMs: number;
  baselineWindowMs: number;
  minQuoteVolumeUsd: number;
  minAbsPriceImpulsePct: number;
  minVolumeImpulse: number;
  minTradeRateImpulse: number;
  minDirectionalTakerRatio: number;
  minHeatScore: number;
  maxSymbols: number;
}

const DEFAULT_CONFIG: FuturesRadarConfig = {
  windowMs: 30_000,
...
interface SymbolState { ticks: TradeTick[]; }

export class FuturesRadarEngine {
  private readonly config: FuturesRadarConfig;
  private readonly states = new Map<string, SymbolState>();

  constructor(config: Partial<FuturesRadarConfig> = {}) {
    this.config = { ...DEFAULT_CONFIG, ...config };
  }

  public ingest(tick: TradeTick): void {
    if (!Number.isFinite(tick.price) || !Number.isFinite(tick.quantity) || tick.price <= 0 || tick.quantity <= 0) return;
  ...
  public scan(now: number): RadarCandidate[] {
  ...
  private evaluateSymbol(symbol: string, ticks: TradeTick[], now: number): RadarCandidate | null {
    ...
    if (recent.length < 4 || baseline.length < 8) return null;
    ...
    for (const t of recent) {
      ...
      totalQuote += q;
      if (!t.isBuyerMaker) buyQuote += q;
    }
    const takerBuyRatio = totalQuote > 0 ? buyQuote / totalQuote : 0.5;
    const direction: ScalpDirection = priceImpulsePct >= 0 ? 'LONG' : 'SHORT';
    const directionalRatio = direction === 'LONG' ? takerBuyRatio : 1 - takerBuyRatio;

    if (quoteVolume < this.config.minQuoteVolumeUsd) return null;
    if (absImpulse < this.config.minAbsPriceImpulsePct) return null;
    if (volumeImpulse < this.config.minVolumeImpulse) return null;
    if (tradeRateImpulse < this.config.minTradeRateImpulse) return null;
    if (directionalRatio < this.config.minDirectionalTakerRatio) return null;

    const heatScore = Math.round(Math.min(100,
      this.score(absImpulse, this.config.minAbsPriceImpulsePct, 30) +
    ...
    if (heatScore < this.config.minHeatScore) return null;
  ...
  private quoteVolume(ticks: TradeTick[]): number {
    return ticks.reduce((sum, t) => sum + t.price * t.quantity, 0);
  }

  private score(value: number, threshold: number, maxPoints: number): number {
    if (threshold <= 0) return maxPoints;
